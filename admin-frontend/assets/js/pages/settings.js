...
socialContainer.addEventListener("click", function (event) {
 ...
 document.getElementById("settingsUpdateBtn").addEventListener("click",async()=>{const id=getId();if(!id)return TL.showToast("Please save settings first to generate a configuration ID.","warning");P.clearErrors(form);const b=document.getElementById("settingsUpdateBtn");P.setBusy(b,true);try{const r=await TL.Settings.updateWebsiteSettings(id,files());capture(r);TL.showToast("Website settings updated.","success");}catch(err){if(err instanceof TL.Api.ApiValidationError)P.showValidation(form,err.errors);TL.showToast(err.message,"error");}finally{P.setBusy(b,false);}});
